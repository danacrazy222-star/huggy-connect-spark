...
import { CheckCircle, X, Wallet, CreditCard } from "lucide-react";
...
interface ShopConfirmPopupProps {
  ...
  quantity: number;
  onConfirm: (useWallet: boolean) => void;
  onCancel: () => void;
  isRTL: boolean;
}
...
export function ShopConfirmPopup({ show, pkg, quantity, onConfirm, onCancel, isRTL }: ShopConfirmPopupProps) {
  ...
  const canAffordWallet = points >= totalPointsCost;
  const walletCredit = Math.floor(points / 1000);
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-6" onClick={onCancel}>
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="bg-card border border-border rounded-2xl p-5 w-full max-w-sm shadow-lg">
            ...
            <div className="space-y-2">
              {/* Pay with wallet */}
              <button
                onClick={() => { if (canAffordWallet) { playPurchaseConfirm(); onConfirm(true); } }}
                disabled={!canAffordWallet}
                className="w-full py-3 rounded-xl font-display font-bold text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-30 disabled:cursor-not-allowed border border-primary/40 bg-primary/10 text-primary hover:bg-primary/20"
              >
                <Wallet className="w-4 h-4" />
                {canAffordWallet
                  ? `${t("payWithWallet")} — ${totalPointsCost.toLocaleString()} ${t("xpPoints")}`
                  : `${t("walletBalance")}: ${points.toLocaleString()} pts ($${walletCredit})`
                }
              </button>

              {/* Pay with real money */}
              <button
                onClick={() => { playPurchaseConfirm(); onConfirm(false); }}
                className="w-full py-3 rounded-xl font-display font-bold text-sm text-primary-foreground hover:brightness-110 transition-all flex items-center justify-center gap-2"
                style={{ background: "linear-gradient(180deg, hsl(45 100% 50%), hsl(40 100% 40%))", boxShadow: "0 0 20px rgba(255,200,0,0.2)" }}
              >
                <CreditCard className="w-4 h-4" />
                {t("payExternal")} — ${totalPrice}
              </button>
            </div>
          </motion.div>
        </motion.div>
