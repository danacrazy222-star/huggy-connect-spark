    ...
    <div className="min-h-screen bg-premium-gradient stars-bg pb-20" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="px-4 space-y-6">
        <div className="text-center">
          ...
          <h2 className="font-display text-xl font-bold text-gold-gradient">{t("aiTarotReading")}</h2>
          <p className="text-sm text-muted-foreground mt-1">{t("selectCardReveal")}</p>
          <div className={cn("inline-flex items-center gap-2 bg-muted/50 rounded-full px-3 py-1 mt-2", isRTL && "flex-row-reverse")}>
        ...
        <div className="grid grid-cols-3 gap-3">
          {tarotCards.map((card, i) => (
            <motion.button key={i} onClick={() => handleCardSelect(i)} disabled={tarotTickets <= 0 || revealed}
              whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              className={`relative aspect-[2/3] rounded-xl border overflow-hidden transition-all ${
                selectedCard === i ? "border-primary shadow-gold" : "border-border hover:border-accent/50"
              ...
              <AnimatePresence>
                {selectedCard === i && revealed ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-[10px] text-muted-foreground mt-2">{t("card")} {i + 1}</span>
                  </div>
                )}
              </AnimatePresence>
            </motion.button>
